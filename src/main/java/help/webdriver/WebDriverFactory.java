...
public class WebDriverFactory {
    ...
    public static class CustomHttpClientFactory extends OkHttpClient.Factory {
        ...
        public HttpClient createClient(final URL url) {
            return clientMap.computeIfAbsent(url.toString(), strUrl -> super.createClient(url));
        }
    ...
    private static RemoteWebDriver buildRemoteWebDriver(String wdUrl, Capabilities caps) {
        ...
        if (useCustomHttpClientFactory) {
            rwdSupplier = () -> {
                ...
                HttpClient.Factory clientFactory = new CustomHttpClientFactory(connTimeout, soTimeout);
